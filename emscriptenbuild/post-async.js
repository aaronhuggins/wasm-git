...
const ENVIRONMENT_IS_DENO = typeof Deno === "object" && typeof Deno.version === "object" && typeof Deno.version.deno === "string";
...
if (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_DENO) {
...
if (ENVIRONMENT_IS_WEB) {
    ...
    Object.assign(Module, {
        emscriptenhttpconnect: async function (url, buffersize, method, headers) {
            ...
            return Promise.resolve(httpConnectionNo++)
        },
        emscriptenhttpwrite: function (connectionNo, buffer, length) {
            const connection = emscriptenhttpconnections[connectionNo];
            const buf = new Uint8Array(Module.HEAPU8.buffer, buffer, length).slice(0);
        ...
        emscriptenhttpread: async function (connectionNo, buffer, buffersize) {
            const connection = emscriptenhttpconnections[connectionNo];
            if (!connection.res) {
                const response = connection.content
                    ? await fetch(connection.req.url, {
                            ...connection.erq.init,
                            body: connection.content
                        })
                    : await fetch(connection.req.url, connection.req.init)
                connection.res = await response.arrayBuffer()
            }
            let bytes_read = connection.res.byteLength - connection.resultbufferpointer;
            if (bytes_read > buffersize) {
                bytes_read = buffersize;
            }
            const responseChunk = new Uint8Array(connection.res, connection.resultbufferpointer, bytes_read);
            writeArrayToMemory(responseChunk, buffer);
            connection.resultbufferpointer += bytes_read;
